...
skunkBtn.addEventListener("click", function(event){
 for(let x = 0; x < allImages.length; x++){
   allImages[x].src = "skunk.webp"
 }
})
...
for(let i = 0; i <allImages.length; i++){
  allImages[i].addEventListener("mouseover", function(event){
    allImages[i].style.width = "110%"
  })
  allImages[i].addEventListener("mouseout", function(event){
...
for(let k = 0; k < paragraphs.length; k++){
  paragraphs[k].addEventListener("click", function(event){
    paragraphs[k].style.fontFamily = "Indie Flower, cursive"
  })
}
